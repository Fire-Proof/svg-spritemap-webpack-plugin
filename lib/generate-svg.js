...
function cssNamespaceFromIndex(index) {
  let base = index.toString(26);
  return base.split('').map((char) => {
    let charCode = char.charCodeAt(0);
    if (charCode > 47 && charCode < 58) {
      return String.fromCharCode(charCode + 49) // convert numbers to a-j
    }
    if (charCode > 96 && charCode < 113) {
      return String.fromCharCode(charCode + 10) // shift remaining chars 10
    }
  }).join('');
};
...
module.exports = (files = [], options = {}) => {
    ...
    const select = xpath.useNamespaces({'svg': 'http://www.w3.org/2000/svg', 'xlink': "http://www.w3.org/1999/xlink"});
    ...
    return new Promise((resolve, reject) => {
        ...
        Promise.all(files.map((file) => new Promise((resolve) => {
            ...
            items.forEach((item, index) => {
                ...
                // apply css namespace
                const nsId = cssNamespaceFromIndex(index);
                let ns = [];
                select('//*[@class]', sprite).forEach((elem) => {
                    const originalClasses = elem.getAttribute('class')
                      .split(' ')
                      .filter(classname => classname.trim());
                    const classes = originalClasses.map((classname) => nsId + '-' + classname);
                    elem.setAttribute('class', classes.join(' '));
                    ns.push(...originalClasses.map((oc, i) => [oc, classes[i]]))
                });

                select('//svg:style', sprite).forEach((style) => {
                    style.textContent = ns.reduce((textContent, [originalClass, newClass]) => {
                        return textContent.replace('.' + originalClass, '.' + newClass);
                    }, style.textContent)
                });
